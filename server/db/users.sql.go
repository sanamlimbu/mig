...
const getLastReadMessage = `-- name: GetLastReadMessage :one
...
type GetLastReadMessageParams struct {
	SenderID    pgtype.UUID
	RecipientID pgtype.UUID
}

func (q *Queries) GetLastReadMessage(ctx context.Context, arg GetLastReadMessageParams) (Message, error) {
	row := q.db.QueryRow(ctx, getLastReadMessage, arg.SenderID, arg.RecipientID)
	var i Message
	err := row.Scan(
		&i.ID,
		&i.SenderID,
